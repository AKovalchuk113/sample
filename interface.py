...
def cv2_to_pixbuf(frame):
    ...
    h, w, ch = rgb.shape
    pixbuf = Gdk.pixbuf_new_from_data(
        rgb.tobytes(),
        Gdk.Colorspace.RGB,
        False,
        8,
    ...
    def run(self):
        ...
        while self.running:
            ...
            if pixbuf is not None:
                # Отправляем кадр в главный поток через GLib.idle_add
...
class MainWindow(Gtk.Window):
    def __init__(self, video_paths):
        ...
        self.cells = [None] * self.num_cells   # список VideoWidget
        ...
        # Назначаем видео виджетам (первым N видео, остальные пустые)
        self.assign_videos()

        # При клике по маленькому виджету меняем его с центральным
        for idx, cell in enumerate(self.cells):
            if idx != 4 and cell.video_path is not None:
                # event_box для обработки клика (DrawingArea не излучает button-press-event по умолчанию)
                evbox = Gtk.EventBox()
                evbox.add(cell)
                ...
                self.cells[idx] = evbox  # сохраняем как EventBox (внутри лежит VideoWidget)
            elif idx == 4:
                # для центрального тоже делаем кликабельным? не нужно
                pass
    ...
    def assign_videos(self):
        ...
        for i in range(self.num_cells):
            if i < len(self.video_paths):
                ...
                widget = self.cells[i] if not isinstance(self.cells[i], Gtk.EventBox) else self.cells[i].get_child()
    ...
    def on_small_click(self, eventbox, event, idx):
        ...
        # Если видео в центре уже играет – останавливаем поток
        ...
        if not self.start_btn.get_sensitive():  # значит всё запущено
